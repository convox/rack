...
func FormationList(rw http.ResponseWriter, r *http.Request) *HttpError {
	app := mux.Vars(r)["app"]
...
func FormationSet(rw http.ResponseWriter, r *http.Request) *HttpError {
	vars := mux.Vars(r)
	app := vars["app"]
	...
	if ae, ok := err.(awserr.Error); ok {
		if ae.Code() == "ValidationError" {
			...
			case strings.Index(ae.Error(), "No updates are to be performed") > -1:
				return HttpErrorf(403, "no updates are to be performed: %s", app)
			case strings.Index(ae.Error(), "can not be updated") > -1:
				return HttpErrorf(403, "app is already updating: %s", app)
			}
		}
	}
