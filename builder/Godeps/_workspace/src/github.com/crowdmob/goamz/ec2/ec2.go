...
import (
	...
	"github.com/convox/kernel/builder/Godeps/_workspace/src/github.com/crowdmob/goamz/aws"
)
...
type AvailabilityZone struct {
	...
	Messages []string `xml:"messageSet>item"`
}

func (ec2 *EC2) DescribeAvailabilityZones(zoneIds []string, filter *Filter) (resp *DescribeAvailabilityZonesResp, err error) {
	params := makeParams("DescribeAvailabilityZones")
	addParamsList(params, "ZoneName", zoneIds)
	filter.addParams(params)
...
type CreateVpcResp struct {
	RequestId string    `xml:"requestId"`
	Vpc       VpcStruct `xml:"vpc"`
}

func (ec2 *EC2) CreateVpc(cidr string) (resp *CreateVpcResp, err error) {
...
type VpcSubnet struct {
	AvailabilityZone string `xml:"availabilityZone"`
	AvailableIPs     int    `xml:"availableIpAddressCount"`
	CidrBlock        string `xml:"cidrBlock"`
	Default          bool   `xml:"defaultForAz"`
	Public           bool   `xml:"mapPublicIpOnLaunch"`
	State            string `xml:"state"`
	SubnetId         string `xml:"subnetId"`
	Tags             []Tag  `xml:"tagSet>item"`
	VpcId            string `xml:"vpcId"`
...
type CreateSubnetResp struct {
	RequestId string    `xml:"requestId"`
	Subnet    VpcSubnet `xml:"subnet"`
}

func (ec2 *EC2) CreateSubnet(vpcId string, availabilityZone string, cidr string) (resp *CreateSubnetResp, err error) {
...
func (ec2 *EC2) DeleteSubnet(subnetId string) (resp *DeleteSubnetResp, err error) {
	params := makeParams("DeleteSubnet")
	params["SubnetId"] = subnetId

	resp = &DeleteSubnetResp{}
	err = ec2.query(params, resp)
	if err != nil {
		return nil, err
	}
