...
func (p *Provider) AppLogs(name string, opts structs.LogsOptions) (io.ReadCloser, error) {
	...
	// go logs.Subscribe(ctx, w, name, helper
...
func subscribeLogs(ctx context.Context, w io.WriteCloser, stream string, opts structs.LogsOptions) {
	...
	// logs.Subscribesub(sctx, ch, time.Now().UTC().Add(-1*helpers.DefaultDuration(opts.Since, 0)), helpers.DefaultBool(opts.Follow, true))
