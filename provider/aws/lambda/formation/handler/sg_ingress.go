...
func SGIngressApply(req Request) (string, map[string]string, error) {
	...
	for _, ipStr := range newIpStrs {
		newIps = append(newIps, &ec2.IpRange{
			CidrIp:      aws.String(strings.TrimSpace(ipStr)),
	...
	return fmt.Sprintf("ingress-%s", *sgId), nil, nil
}
