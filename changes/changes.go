...
func syncUntilStable(dir string, ignore []string, ch chan Change, prev map[string]time.Time) (map[string]time.Time, error) {
	for i := 0; i < 10; i++ {
...
func notify(ch chan Change, from, to map[string]time.Time, base string, ignore []string) bool {
