...
// osMkdirAll and osWriteFile allow override in tests (see kaniko_test.go)
...
func loginForKaniko(auth map[string]struct {
	...
	// attempt to create Docker config directory, ignore errors (e.g., permission denied)
	_ = osMkdirAll(configDir, 0755)
	...
	if err := osWriteFile(configPath, authJSON, 0600); err != nil {
...
func (bb *Build) buildWithKaniko(path, dockerfile string, tag string, env map[string]string) error {
	...
	err = bb.Exec.Run(bb.writer, "/kaniko/executor", args...)
	if err != nil {
