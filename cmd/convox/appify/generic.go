...
func (g *GenericApp) Appify() error {
	...
	if err := writeAsset("docker-compose.yml", fmt.Sprintf("appify/templates/%s/docker-compose.yml", g.AppKind), nil); err != nil {
		return err
	}

	if err := writeAsset(".dockerignore", fmt.Sprintf("appify/templates/%s/.dockerignore", g.AppKind), nil); err != nil {
		return err
	}
